...
import { addDocumentById } from "../../../services/FirebaseService";
import { googleProvider, auth } from "../../../services/firebase";
import { CustomerLoginContext } from "../../../context/CustomerLoginContext";
import { Dialog } from '@mui/material';
import { ContextLogin } from "../../../context/LoginContext";
import { ROLES } from "../../../utils/Constants";
function ModalLogin({ setLoginModal, loginModal,setHidden })   {
const { setIsLogin } = useContext(CustomerLoginContext);

  const customerList = useContext(ContextLogin);

  const signInWithGoogle = async () => {
    try {
      const result = await signInWithPopup(auth, googleProvider);
     const isLogin =  customerList.find(a => a.id == result.user.email);
     let login;
     if(!isLogin) {
       const newCustomer = {
      ...
      await addDocumentById('Customers', result.user.email, newCustomer);
       login = newCustomer
     }else {
       login = isLogin ;
     } 
